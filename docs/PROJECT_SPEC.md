...
- 포인트 **적립 로그** 기록(잔액=로그 합계, 5년 소멸). **기프티콘 교환은 구현됨**(포인트샵 → 차감 → 관리자 수동 발송). 현금 출금은 미구현.
- 회원 탈퇴 + 개인정보 삭제 (법적 필수)
- 개인정보처리방침 / 이용약관 페이지
...
- ~~GPS/현재위치 기반 서비스~~ → **In Scope(제한적)**: "현재 위치" 버튼은 **지도 이동 보조 용도로만**(브라우저 Geolocation), **좌표를 서버/DB에 저장하지 않음**. 운영 시 위치정보법/개인정보처리방침 검토 필요.
- ~~사장님 직접 등록 / 사업자 전환 / 내가게 관리 대시보드~~ → **In Scope로 편입(Phase 7)**
- 사업자등록번호 검증(국세청 API) — **미사용 유지**(서류 업로드 + 관리자 수동 승인으로 대체)
- 푸시 알림(Push) / 네이티브 앱
- ~~포인트 → 기프티콘 교환~~ → **In Scope로 편입**(포인트샵·교환·정산). 현금 **출금은 여전히 Out**.
- 아바타/치장/닉네임 특수효과 게이미피케이션
- ~~관리자 대시보드(가입자 통계, 접속 통계 등)~~ → **In Scope로 편입**(`/admin` 현황·활동분석·영업·정산 등)
- ~~수익화(구독/스폰서 핀/광고)~~ → **In Scope로 편입(M0~M6, 2026-06)**: 전환 로깅(M0)·스폰서 정액 광고(M1-A)·영업 리드(M1-B)·토스 빌링키 정기결제(M2)·사장님 쿠폰(M3)·프로 플랜(M4)·기프티콘 제휴 정산(M5)·사장님 관리 대시보드(M6). **실 과금은 토스 테스트키 검증 단계**(실 빌링 계약 후 ON). 상세는 [`HANDOFF.md`](HANDOFF.md)·[`BUSINESS_OVERVIEW.md`](BUSINESS_OVERVIEW.md).
  - **M7(L2): 마감임박 떨이 픽업 예약** → **In Scope로 편입(2026-06)**: 마감임박 세일을 소비자가 앱에서 선점(예약)하고 매장 픽업. **v1=현장결제**(매장 지불) — 앱은 선점/픽업확인/노쇼만(쿠폰 use 자기처리 모델). 플랫폼 수수료(10%)는 픽업 완료 시 확정되는 미수금(원장 기록). **선결제(토스 일반결제)·자동 정산분배·전자상거래법 청약철회·식품표시는 Phase 2**(⚠️ 컴플라이언스 선행). 출처: [`MONETIZATION_PM_BRIEF.md`] L2.
...
- **위치정보 최소 사용**: 기본은 검색어 → 지오코딩으로 지도 이동. 추가로 "현재 위치" 버튼은 브라우저 Geolocation으로 **지도만 이동**(좌표 미저장). 단말 GPS 좌표를 수집/저장하지는 않는다.
...
### Phase 3 — 사용자 제보
- 세일 제보: 사진(**최대 10장 · 슬라이드 표시**) + 내용 + 세일가 + 수량 + 만료시간(1h/2h/마감까지/**직접 설정**)
  - **원탭(최소) 제보 경로**(콜드스타트 개편, [`THEME_MAP_BENCHMARK_PM_BRIEF.md`](THEME_MAP_BENCHMARK_PM_BRIEF.md) P0-1): 가게 선택만 필수 — 사진·가격·내용·수량은 **선택**(미입력 시 기본 제목 "오늘 세일해요" · 가격 null='세일중' 표시 · 만료=가게 마감까지, migration 44). 어뷰징은 레이트리밋 + 같은 가게·항목 하루 1회 + 활성 중복 409 + 신고 자동숨김으로 방어. 상세 폼(가게 상세의 세일 탭)은 기존 필수 규칙(사진·내용·가격) 유지.
...
- **7b 메뉴(상품) 관리/권한**:
  ...
  - **계정 정지(ban)**: 신고 누적/관리자 판단으로 작성자 `User.status=banned` → 모든 작성 차단(세션 게이트). `/admin/users` 에서 해제.
- **가게 등록 편의**: 주소 직접 입력 외에 **지도에서 좌표 선택**(역지오코딩으로 주소 자동 기입) 지원. 위치정보 미수집 원칙 유지(사용자가 직접 찍은 가게 위치이며 단말 GPS 미사용).
- **7c 가게 상세 UI**: 상단 **메인 사진(배너, `Store.bannerUrl`)** + 가게명 + 탭(상품/세일/공지/리뷰).
- **동영상(외부 임베드)**: 배너/메뉴/세일/리뷰에 YouTube 등 **외부 링크 임베드**(직접 업로드 X, 저장 부담 없음). `bannerVideoUrl`/`videoUrl`.
- **소개 페이지**(`/about`): Apple식 긴 스크롤 랜딩 + 이미지 hover 확대.
