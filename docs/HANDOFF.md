...
## 4. 데이터 모델 (Prisma, migrations 0~29 적용 완료)
...
- **migration 29(M1-A)**: `Sponsorship`(id, storeId, plan(기본 bundle), region, status[trial|active|expired|canceled], priceKrw(기본 29800), trialEndsAt, startsAt, endsAt, note?, @@index[status,endsAt]/[storeId]) — 스폰서 정액 광고. 한 가게가 기간 갱신하며 여러 행(이력) 가능, 노출은 endsAt 기준. Store cascade.
