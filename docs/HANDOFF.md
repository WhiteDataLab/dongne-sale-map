...
- **회원 정보 관리(`/admin/members`)**: 가입일·계정(provider/전화)·닉네임·**적립포인트(PointLog 5년 합계 groupBy)** 목록 + **계정 잠금/해제(`lockUser`/`unlockUser`)**·**강제 탈퇴(`forceDeleteUser` — 콘텐츠 익명화+탈퇴로그+User 삭제)**. 관리자 계정은 보호. 파괴적 액션은 `ConfirmSubmit`로 확인.
