...
- 잔여 위험(낮음): 토큰 maxAge 동안 닉네임/이미지 지연, `/api/feed`·`/api/stores` 공개(단 `take` 상한으로 쿼리비용 제한), 계정연결 `link_uid`는 httpOnly+인증세션 기반(XSS 없으면 안전), `hashCode`/`hashContact`의 `AUTH_SECRET ?? "salt"` 폴백(운영선 AUTH_SECRET 항상 설정 전제), 레이트리밋 분산화·CSP nonce 축소는 후속.
