...
- **내 리뷰 관리 + 좋아요/싫어요(migration 20, `ReviewReaction`)**: 마이페이지에 **내가 쓴 리뷰 목록**(가게명·별점·내용·일시·👍/👎수)+**삭제**(`DELETE /api/reviews/[id]`, 작성자/관리자, 포인트 회수). 리뷰 **좋아요/싫어요 토글**(`POST /api/reviews/[id]/react`, 1인 1반응, `ReviewReactions` 컴포넌트). ReviewDTO에 likeCount/dislikeCount/myReaction/isMine 추가.
- **가게 상세 리뷰 수정/삭제 + 작성일 라벨**: 가게 상세 리뷰 탭에서 **내 리뷰(isMine)에 수정/삭제 버튼** 노출. 수정=`PATCH /api/reviews/[id]`(작성자/관리자, 별점·내용·사진 갱신, 포인트는 작성시점 기준이라 미변동, 교체/삭제된 이전 사진 스토리지 정리). `ReviewForm`에 `review` prop으로 **수정 모드**(태그 프리셋·구매메뉴 퍼널 대신 본문 직접 편집, 기존 사진 유지/삭제+새 사진 추가). **마이페이지(`/account`)도 동일 수정** — `MyReviewItem`(클라)에서 `ReviewForm` 수정 모드를 인라인으로 열고(삭제는 기존 `DeleteReviewButton`), 리뷰 select에 `storeId`/`photoUrls` 추가. 작성일은 정확한 시각 대신 **KST 달력일 기준 한글 구간**(`reviewDateLabel`/`kstDaysAgo`로 시각 아닌 날짜경계 차이 계산): 오늘/어제/그제(0·1·2일)·3~6 이번주·7~13 저번주·14~27 이번달·28~59 저번달·60~364 `N개월 전`(2~11)·365+ `N년 전`(1·2·3…).
