...
- [x] **7b-2 계정 정지(ban)**: 관리자가 신고 큐에서 **작성자 정지** + `/admin/users` 정지 해제. 정지 계정은 모든 작성 차단(세션 게이트에서 status 확인)
- [x] **가게 등록 지도 좌표 선택**: 등록 폼에서 "지도에서 위치 선택" → 탭한 지점 역지오코딩으로 **주소 자동 기입** + 그 좌표로 등록(지오코딩 생략)
- [x] **사진 편집(자르기/색깔펜)**: 세일 제보 사진 썸네일 "✏️ 편집" → 캔버스 에디터(펜 색상·굵기, 영역 드래그 자르기) → JPEG로 저장 후 업로드 (라이브러리 미사용)
