    ...
    <div>
      ...
      <div className="mb-4 grid grid-cols-3 gap-2 text-center">
        <div className="rounded-xl bg-indigo-50 p-3">
          <p className="text-[11px] text-ink-3">집행 중</p>
          <p className="text-lg font-bold text-indigo-700">{activeCount}</p>
        </div>
        <div className="rounded-xl bg-green-50 p-3">
          <p className="text-[11px] text-ink-3">미정산 집행액</p>
          <p className="text-lg font-bold text-green-700">{totalSpent.toLocaleString("ko-KR")}원</p>
        </div>
        <div className="rounded-xl bg-surface-2 p-3">
          <p className="text-[11px] text-ink-3">총 캠페인</p>
          <p className="text-lg font-bold text-ink-2">{campaigns.length}</p>
        </div>
      </div>

      {campaigns.length === 0 ? (
        <p className="rounded-xl border border-dashed border-line p-8 text-center text-sm text-ink-3">
          아직 성과형 광고 캠페인이 없어요.
        </p>
      ) : (
        <ul className="flex flex-col gap-2">
          {campaigns.map((c) => (
            <li key={c.id} className="rounded-xl border border-line p-3">
              ...
              <p className="text-[11px] text-ink-3">{c.store.address}</p>
              ...
              <p className="mt-0.5 text-xs">
                ...
                <span className="text-ink-3"> / 예산 {c.budgetKrw.toLocaleString("ko-KR")}원</span>
              </p>
            </li>
          ))}
        </ul>
