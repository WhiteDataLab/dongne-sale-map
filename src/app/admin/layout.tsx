...
function AdminGroup({ label, links }: { label: string; links: [string, string][] }) {
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
      <span className="w-20 shrink-0 text-xs font-bold text-ink-3">{label}</span>
      {links.map(([href, text]) => (
        <Link key={href} href={href} className="font-medium text-ink-2 hover:text-ink">
          {text}
        </Link>
      ))}
    </div>
...
export default async function AdminLayout({
  ...
  if (!session) {
    return (
      <div className="h-full overflow-y-auto p-6 text-center text-sm text-ink-2">
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-2xl p-4">
        <header className="mb-4 border-b border-line-2 pb-3">
          <div className="mb-2 flex items-center justify-between">
            <Link href="/admin" className="font-extrabold tracking-tight text-ink">
              관리 콘솔
            </Link>
            <Link href="/" className="text-xs font-medium text-ink-3">
              ← 지도
            </Link>
          </div>
