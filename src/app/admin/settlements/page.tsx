...
export default async function AdminSettlements() {
  ...
  if (dbError) {
    return <p className="py-10 text-center text-sm text-gray-400">정산 정보를 불러오지 못했어요.</p>;
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-xl border border-gray-200 p-3">
          <p className="text-[11px] text-gray-400">교환 건수</p>
          <p className="text-lg font-bold">{rows.length.toLocaleString("ko-KR")}건</p>
        </div>
        <div className="rounded-xl border border-gray-200 p-3">
          <p className="text-[11px] text-gray-400">포인트 사용(명목 액면)</p>
        ...
        <div className="rounded-xl border border-rose-200 bg-rose-50/50 p-3">
          ...
          <p className="mt-0.5 text-[10px] text-gray-400">미정산 {won(unsettledCost)} · 정산완료 {won(settledCost)}</p>
        </div>
        <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-3">
          ...
          {noCost > 0 && <p className="mt-0.5 text-[10px] text-gray-400">원가 미입력 {noCost}건 제외</p>}
        </div>
      </div>
      ...
      <section className="flex flex-col gap-2">
        ...
        {partners.length === 0 ? (
          <p className="py-4 text-center text-xs text-gray-400">집계할 교환이 없어요.</p>
        ) : (
          <ul className="flex flex-col gap-1.5">
            {partners.map(([name, g]) => (
              <li key={name} className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm">
      ...
      <section className="flex flex-col gap-2">
        ...
        {settleList.length === 0 ? (
          <p className="py-4 text-center text-xs text-gray-400">발송 완료된 교환이 없어요.</p>
        ) : (
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  {r.settledAt ? (
                    ...
                    <form action={settleRedemption}>
                      <input type="hidden" name="id" value={r.id} />
                      <button className="shrink-0 rounded-lg border border-gray-300 px-2.5 py-1 text-[11px] font-medium text-gray-600 hover:bg-gray-100">
                        정산완료 처리
                      </button>
                    </form>
                  )}
