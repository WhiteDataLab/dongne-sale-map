...
const STATUS_BADGE: Record<string, string> = {
  ...
  contacted: "bg-blue-100 text-brand-ink",
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-xs text-ink-3">주인 없는 가게 · 활동도순 · 최대 200</span>
      </div>
      ...
      <form method="get" className="flex flex-wrap items-center gap-2">
        ...
        <Link
          href={exportHref}
          prefetch={false}
          className="rounded-lg border border-line px-3 py-1.5 text-sm font-medium text-ink-2 hover:bg-surface-2"
        >
          ⬇ CSV
        </Link>
      </form>
      ...
      {dbError ? (
        <p className="py-10 text-center text-sm text-ink-3">리드를 불러오지 못했어요 (DB 연결 확인).</p>
      ) : leads.length === 0 ? (
        <p className="py-10 text-center text-sm text-ink-3">조건에 맞는 리드가 없어요.</p>
      ) : (
            ...
            <li key={l.id} className="rounded-xl border border-line p-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 truncate text-xs text-ink-3">{l.address}</p>
                  <p className="text-xs text-ink-3">
                  ...
                  <p className="mt-1 text-[11px] text-ink-3">
                    ...
                    <span className="text-ink-4"> (최근 30일)</span>
                  </p>
                </div>
                <div className="shrink-0 text-center">
                  ...
                  <p className="mt-0.5 text-[10px] text-ink-3">리드점수</p>
                </div>
              </div>
              ...
              <form action={setLeadStatus} className="mt-2 flex flex-wrap items-center gap-2 border-t border-gray-50 pt-2">
                ...
                />
                <button className="rounded-lg border border-line px-3 py-1 text-xs font-medium text-ink-2 hover:bg-surface-2">
                  저장
                </button>
              </form>
            </li>
