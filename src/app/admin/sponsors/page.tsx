...
const STATUS_BADGE: Record<string, string> = {
  ...
  canceled: "bg-surface-2 text-ink-3 line-through",
};
...
export default async function AdminSponsors({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {dbError && (
        <p className="py-10 text-center text-sm text-ink-3">데이터를 불러오지 못했어요 (DB 연결 확인).</p>
      )}
      ...
      <section className="rounded-xl border border-line p-3">
        ...
        {q && (
          <ul className="mt-2 flex flex-col gap-1.5">
            {matches.length === 0 ? (
              <li className="py-3 text-center text-xs text-ink-3">
              ...
              matches.map((m) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-xs text-ink-3">{m.address}</p>
                  </div>
      ...
      <section className="flex flex-col gap-2">
        ...
        {sponsors.length === 0 && !dbError ? (
          <p className="py-10 text-center text-sm text-ink-3">아직 스폰서가 없어요.</p>
        ) : (
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-ink-3">
                      ...
                      {s.daysLeft >= 0 ? (
                        ...
                        <span className="text-ink-3">만료됨</span>
                      )}
                    </p>
                  </div>
                </div>
                  ...
                  {(s.status === "trial" || s.status === "active") && (
                    <>
                      ...
                      <form action={cancelSponsor}>
                        <input type="hidden" name="id" value={s.id} />
                        <button className="rounded-lg border border-line px-3 py-1 text-xs font-medium text-red-500 hover:bg-red-50">
                          취소
                        </button>
                      </form>
                    </>
