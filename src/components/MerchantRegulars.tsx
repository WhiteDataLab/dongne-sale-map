...
const SEG_META: Record<Segment, { label: string; cls: string }> = {
  ...
  dormant: { label: "휴면", cls: "bg-gray-200 text-gray-500" },
};
  ...
  if (gate) {
    return (
      <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-4 text-center">
        ...
        <p className="mt-1 text-xs text-gray-500">누가 우리 단골인지, 요즘 안 오는 손님이 누군지 확인하고 다시 부를 수 있어요.</p>
    ...
    <div className="flex flex-col gap-3">
      ...
      {rows.length === 0 ? (
        ...
        <ul className="flex flex-col gap-1.5">
          {rows.map((r) => (
            <li key={r.userId} className="flex items-center gap-2.5 rounded-lg border border-gray-100 p-2">
              <div className="size-9 shrink-0 overflow-hidden rounded-full bg-gray-100">
                {r.img ? (
                  ...
                  <div className="flex size-full items-center justify-center text-sm text-gray-400">🙂</div>
                )}
              </div>
              ...
              <span className="shrink-0 text-sm font-bold text-gray-700">{r.score}점</span>
            </li>
          ))}
        </ul>
      )}
  ...
  return (
    <div className="rounded-xl border border-indigo-200 bg-indigo-50/40 p-3">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={() => setOpen(false)} className="text-xs text-gray-400">닫기</button>
      </div>
      <div className="mt-2 flex flex-col gap-1.5">
        <label className="text-xs text-gray-500">대상 세그먼트</label>
          ...
          {(["at_risk", "dormant", "active"] as Segment[]).map((s) => (
            <button
              ...
              className={[
                ...
                segment === s ? "bg-indigo-600 text-white" : "bg-white text-gray-500 border border-gray-200",
              ].join(" ")}
            >
        ...
        <input
          ...
          className="mt-1 w-full rounded-md border border-gray-200 px-2.5 py-2 text-sm focus:border-indigo-400 focus:outline-none"
        />
        <input
          value={condition}
          ...
          className="w-full rounded-md border border-gray-200 px-2.5 py-2 text-sm focus:border-indigo-400 focus:outline-none"
        />
        ...
        <p className="text-[10px] text-gray-400">대상 단골의 쿠폰함에 바로 지급되고, 즐겨찾기 손님에겐 알림도 가요. (월 2회)</p>
      </div>
    </div>
  );
}
