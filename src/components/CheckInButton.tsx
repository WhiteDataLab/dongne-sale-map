...
export function CheckInButton({ checkedToday }: { checkedToday: boolean }) {
  ...
  const checkIn = async () => {
    ...
    try {
      ...
      const bonus =
        (data.awarded?.weekly ? " (주간 +20P)" : "") + (data.awarded?.monthly ? " (월간 +50P)" : "");
      setMsg(`출석 완료! +${data.awarded?.total ?? 10}P${bonus} · ${data.streak}일 연속 🔥`);
      router.refresh();
    } catch {
