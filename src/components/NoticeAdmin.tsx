  ...
  return (
    <div className="flex flex-col gap-4">
      <section className="rounded-xl border border-gray-200 p-4">
        ...
        <div className="flex flex-col gap-2">
            ...
            {(["notice", "event"] as const).map((k) => (
              <button
                ...
                className={[
                  ...
                  kind === k ? "border-blue-600 bg-blue-600 text-white" : "border-gray-200 text-gray-600",
                ].join(" ")}
              >
                {KIND_LABEL[k]}
              </button>
          ...
          <button
            ...
            onClick={create}
            disabled={busy}
            className="self-start rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white disabled:bg-gray-300"
          >
            {busy ? "등록 중…" : "등록"}
          </button>
        </div>
      ...
      <ul className="flex flex-col gap-2">
        {notices.length === 0 && <p className="text-sm text-gray-400">등록된 공지가 없어요.</p>}
        {notices.map((n) => (
          <li key={n.id} className="rounded-xl border border-gray-200 p-3">
            <div className="flex items-center gap-2">
              ...
              {!n.active && <span className="text-xs text-gray-400">숨김</span>}
              <span className="ml-auto text-xs text-gray-400">
                {new Date(n.createdAt).toLocaleDateString("ko-KR")}
              </span>
            </div>
            <p className="mt-1 font-medium">{n.title}</p>
            <p className="mt-0.5 whitespace-pre-wrap text-sm text-gray-600">{n.body}</p>
            <div className="mt-2 flex flex-wrap gap-3 text-xs">
              <button type="button" onClick={() => patch(n.id, { pinned: !n.pinned })} className="text-amber-600">
                {n.pinned ? "고정 해제" : "상단 고정"}
              </button>
              <button type="button" onClick={() => patch(n.id, { active: !n.active })} className="text-gray-600">
                {n.active ? "숨기기" : "노출하기"}
              </button>
