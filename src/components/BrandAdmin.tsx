  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-xl border border-gray-200 p-3">
        ...
        {gifts.length === 0 ? (
          <p className="text-xs text-gray-400">먼저 포인트샵에 기프티콘 상품을 등록해 주세요.</p>
        ) : (
          <div className="flex flex-col gap-1.5">
            <input
              ...
              className="rounded-md border border-gray-200 px-2.5 py-2 text-sm"
            />
            <select
              value={giftItemId}
            ...
            <div className="grid grid-cols-3 gap-1.5">
              <label className="text-[11px] text-gray-500">
                CPA단가
                <input type="number" value={cpaKrw} onChange={(e) => setCpaKrw(Number(e.target.value))} className="mt-0.5 w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm" />
              </label>
              <label className="text-[11px] text-gray-500">
                예산
                <input type="number" value={budgetKrw} onChange={(e) => setBudgetKrw(Number(e.target.value))} className="mt-0.5 w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm" />
              </label>
              <label className="text-[11px] text-gray-500">
                1인 한도
                <input type="number" value={perUserLimit} onChange={(e) => setPerUserLimit(Number(e.target.value))} className="mt-0.5 w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm" />
              </label>
            </div>
      ...
      {/* 캠페인 목록 */}
      {campaigns.length === 0 ? (
        <p className="rounded-xl border border-dashed border-gray-200 p-6 text-center text-sm text-gray-400">아직 캠페인이 없어요.</p>
      ) : (
        <ul className="flex flex-col gap-2">
          {campaigns.map((c) => {
            const s = STATUS_LABEL[c.status] ?? { txt: c.status, cls: "bg-gray-100 text-gray-500" };
            ...
            return (
              <li key={c.id} className="rounded-xl border border-gray-200 p-3">
                ...
                <p className="text-[11px] text-gray-400">{c.giftName} · CPA {c.cpaKrw.toLocaleString("ko-KR")}원 · 1인 {c.perUserLimit}회</p>
                <div className="mt-1.5 h-2 w-full overflow-hidden rounded-full bg-gray-100">
                  <div className="h-full bg-indigo-500" style={{ width: `${pct}%` }} />
                </div>
                <p className="mt-1 text-xs">
                  <b className="text-indigo-700">{c.spentKrw.toLocaleString("ko-KR")}원</b>
                  <span className="text-gray-400"> / 예산 {c.budgetKrw.toLocaleString("ko-KR")}원 · 상환 {c.redeemedCount}건</span>
                </p>
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {c.status === "active" && (
                    <button type="button" disabled={busy} onClick={() => op(c.id, "pause")} className="rounded-lg border border-gray-300 px-2.5 py-1 text-xs text-gray-600">일시중지</button>
                  )}
                  {c.status === "paused" && (
                  ...
                  {c.status !== "ended" && (
                    <>
                      ...
                      <button type="button" disabled={busy} onClick={() => op(c.id, "end")} className="rounded-lg border border-gray-300 px-2.5 py-1 text-xs text-gray-500">종료</button>
                    </>
                  )}
                </div>
              </li>
