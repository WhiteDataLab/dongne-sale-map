...
// 전부 카카오 로컬 키워드 검색으로 찾은 실제 단지 좌표. 다만 이 목록은 "이문동의
// 모든 아파트"가 아니라 예시로 고른 일부다 — 카카오 로컬 API엔 "이 구역의 아파트를
// 전부 나열"해주는 카테고리가 없어서(공식 카테고리 코드에 아파트/주거 항목 자체가
// 없음, MT1·CS2 같은 상업·공공시설 카테고리만 있음) 이름을 하나하나 검색해 찾아야
// 한다. 그래서 실제 서비스라면 이 드롭다운은 어디까지나 "자주 찾는 몇 곳 바로가기"
// 정도이고, 목록에 없는 건물(이문e편한세상 등 포함 무엇이든)은 아래 "주소 직접 입력"
// 이 항상 커버한다(같은 카카오 API로 실시간 검색·지오코딩).
// 마지막 한 곳(다세대빌라)은 실존 단지가 아니라 "잔여 주소 직접 입력" 후보4 컨셉을
// 보여주기 위한 손으로 만든 예시일 뿐, 검색으로 찾은 게 아니다.
const SOURCE_OPTIONS: SourcePoint[] = [
  ...
  { label: "이문e편한세상", lat: 37.60452201412, lng: 127.06497244659619 },
  { label: "이문현대아파트", lat: 37.60135114339153, lng: 127.07019244863042 },
  { label: "이문2차푸르지오", lat: 37.6043242718197, lng: 127.07042626672298 },
